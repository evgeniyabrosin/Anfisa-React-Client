...
describe('Test on table of returned variants', () => {
  ...
  it('should open main table | test #4', () => {
    ...
    cy.intercept('POST', '/app/reccnt?ds=xl_PGP3140_wgs_NIST-4_2&rec=2823765').as('returnedVariantsTable')
    ...
    returnedVariantsPage.returnedVariantsTable.returnedVariantsHeader.haveText('[None] chr8:12712859-12712858 insertion')
    returnedVariantsPage.returnedVariantsTable.sampleButton.contains('N - 2').click()
    returnedVariantsPage.returnedVariantsTable.returnedVariantsHeader.haveText('[ANO3] chr11:26589499 deletion')
    returnedVariantsPage.returnedVariantsTable.tableSection.getChildren().contains('Quality').parent().click({ force: true })
  ...
  it('Add dataset and open it in Main Table | test #5', () => {
    ...
    decisionTreesPage.decisionTreeMenu.datasetNameInput.type('Dataset_from_autotests')
    ...
    datasetPage.leftPanel.datasetsListElem.getButtonByText('Dataset_from_autotests').should('exist')
  })
})
