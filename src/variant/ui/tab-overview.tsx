...
export const TabOverview = (): ReactElement => {
	...
	const worstAnnotation = get(variantStore, 'variant[0].rows[3].cells[0][0]', '')
	const canonicalAnnotations = get(variantStore, 'variant[0].rows[4].cells[0][0]', '')
	...
	useEffect(() => {
		...
		setGenotypeData([
			{name: t('variant.proband'), value: get(variantStore, 'variant[0].rows[14].cells[0][0]')},
