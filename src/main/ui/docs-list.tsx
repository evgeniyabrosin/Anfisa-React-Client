...
export const DocsList = ({data, activeName, baseDatasetName, onClick}: Props): ReactElement => {
	...
	return (
		<>
			{baseDatasetName && <InfoTextItem>{t('home.base', {
				...
				return (
					<Box key={doc[0]} >
						...
						{Array.isArray(doc[1]) && (
							<Box>
								{doc[1].map((item) => 
									<InfoTextItem isClickable style={{paddingLeft: baseDatasetName ? 40 : 30}} isActive={activeName === item[0]} key={item[0]} onClick={() => onClick(item)}>{item[0]}</InfoTextItem>
								)}
							</Box>
						)}
					</Box>
