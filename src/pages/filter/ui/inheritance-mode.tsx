...
export const InheritanceMode = observer(({ values, setFieldValue }: FormikProps<IInheritanceFormValues>) => {
  const cachedValues = filterStore.readFilterCondition<IInheritanceFormValues>(FuncStepTypesEnum.InheritanceMode)
  ...
  const handleChangeAsync = async (e: ChangeEvent<HTMLInputElement>, problemGroup: string) => {
    const value = e.target.checked ? [...values.problemGroups, problemGroup] : values.problemGroups.filter(group => group !== problemGroup)
  ...
  useEffect(() => {
    const initAsync = async () => {
      const [problemGroupsData] = await Promise.all([filterStore.fetchProblemGroupsAsync(), fetchStatFuncAsync()])
  ...
  useEffect(() => {
    filterStore.setFilterCondition<IInheritanceFormValues>(FuncStepTypesEnum.InheritanceMode, {
  ...
  return (
    <Form>
      <div className="flex items-center justify-between">
        <p className="text-14 leading-16px font-bold text-grey-blue mt-4">Problem group</p>

        <p className="text-12 text-blue-bright leading-14px cursor-pointer" onClick={handleSetFieldValueAsync}>
          Reset
        </p>
      </div>
      <div className="flex items-center justify-between mt-4">
        {problemGroups.map(problemGroup => (
          <div key={problemGroup} className="flex items-center">
            <Checkbox checked={problemGroupValues.includes(problemGroup)} onChange={e => handleChangeAsync(e, problemGroup)} />
      ...
      <div className="flex items-center justify-between">
        <p className="text-14 leading-14px text-grey-blue">{variantsValues?.length} Selected</p>
        ...
        <p className="text-12 leading-14px text-blue-bright cursor-pointer" onClick={() => setFieldValue('variants', [])}>
          {t('general.clearAll')}
        </p>
      </div>
        ...
        return (
          <div key={variant[0]} className="flex items-center mt-4">
            <Checkbox
              ...
              onChange={e => {
                const value = e.target.checked ? [...variantsValues, variant[0]] : variantsValues.filter(item => item !== variant[0])
