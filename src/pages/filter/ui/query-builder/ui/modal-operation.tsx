...
export const ModalOperation = observer(
  ({ hideModal, index }: IProps): ReactElement => {
    ...
    const deleteStep = (stepIndex: number) => {
      ...
      stepHasAttribute ? changeStep(stepIndex, 'DELETE') : dtreeStore.removeStep(stepIndex)
      ...
      <div ref={ref}>
        <div className="absolute z-50 top-8 w-32 flex flex-col justify-between px-0 py-0 bg-white rounded-md text-14 cursor-pointer shadow-dark">
          <Fragment>
            ...
            <div onClick={() => sendChange(index, 'DUPLICATE')} className="font-normal py-2 px-2 hover:bg-grey-light">
              {t('dtree.duplicate')}
            </div>
