...
export const CustomInheritanceMode = observer(({ setFieldValue }: FormikProps<ICustomInheritanceModeProps>) => {
  const cachedValues = filterStore.readFilterCondition<ICustomInheritanceFormValues>(FuncStepTypesEnum.CustomInheritanceMode)
  ...
  useEffect(() => {
    if (firstSelectValue === '--' && secondSelectValue === '--' && thirdSelectValue === '--') {
      filterStore.setError('Out of choice')
    } else {
      filterStore.setError('')
    }

    filterStore.setFilterCondition<ICustomInheritanceFormValues>(FuncStepTypesEnum.CustomInheritanceMode, {
  ...
  const sendRequestAsync = async (type: string, value: string, multiData?: any[]) => {
    ...
    filterStore.method === GlbPagesNames.Refiner && setFieldValue('scenario', Object.fromEntries(newScenario))
    filterStore.fetchStatFuncAsync('Custom_Inheritance_Mode', params)
  }
