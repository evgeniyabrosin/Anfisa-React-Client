...
const ModalElement = observer(({ close, title }: ModalProps) => {
  ...
  return (
    <PopperTableModal
      ...
      isGenes={true}
    >
      <FilterItemList items={datasetStore.genes.filter(item => item.toLocaleLowerCase().includes(searchValue.toLocaleLowerCase()))} isGenes={true} />
    </PopperTableModal>
  )
})
    ...
    return (
      <div style={{ minWidth: 75 }}>
        <ControlPanelTitle title={title}>
          {toJS(zoneStore.selectedGenes).length > 0 && <PopperButton title={title} ButtonElement={ButtonElementEdit} ModalElement={ModalElement} />}
        </ControlPanelTitle>

        <PopperButton title={title} ButtonElement={ButtonElementAdd} ModalElement={ModalElement} data={zoneStore.selectedGenes} type="add" />
