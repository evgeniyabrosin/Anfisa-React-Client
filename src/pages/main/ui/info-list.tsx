...
export const InfoList = observer(
  (): ReactElement => {
    ...
    const handleClickAsync = async (doc: any, isBaseInfo?: boolean) => {
      ...
      if (doc[2]) {
        if (doc[2].image) {
          dirinfoStore.setInfoFrameLink(`app/dsdoc/${baseDatasetName ?? dirinfoStore.selectedDirinfoName}/${doc[2].image}`)
      ...
      if (isBaseInfo) {
        dirinfoStore.setInfoFrameLink(`app/dsdoc/${dirinfoStore.ancestorsDsInfo[0][0]}/${doc[1]}`)

        return
      }

      dirinfoStore.setInfoFrameLink(`app/dsdoc/${dirinfoStore.selectedDirinfoName}/${doc[1]}`)
    }

    return (
      <div>
        <DocsList activeName={dirinfoStore.activeInfoName} data={docs} onClick={handleClickAsync} />

        {dirinfoStore.ancestorsDsInfo[0] && dirinfoStore.ancestorsDsInfo[0][1] && (
          <DocsList
