...
export const Tag = observer(
  ({ text, isActive, hideCloseIcon, onClick, onRemove }: Props): ReactElement => (
    ...
    >
      <span className={cn(['text-12', 'cursor-pointer', 'leading-14px', 'truncate', isActive ? 'text-white' : 'text-blue-bright'])}>
        {text}
      </span>

      {isActive && !hideCloseIcon && (
        <Icon name="ClosePadded" className="ml-1 cursor-pointer opacity-60" onClick={() => onRemove && onRemove(text)} />
      )}
    </div>
  ),
)
