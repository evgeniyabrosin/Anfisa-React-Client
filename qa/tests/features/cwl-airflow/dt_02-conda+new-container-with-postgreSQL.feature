Feature: CWL-Airflow, DT_02 - Conda+New Container with PostgreSQL
	As a user I want to work with CWL-Airflow on OS with conda and with new container with PostgreSQL
	...
	Scenario: Copy env_example_postgres_noconda
		...
		When the user runs the command in bash shell "cp .env_example_postgres_conda .env"
