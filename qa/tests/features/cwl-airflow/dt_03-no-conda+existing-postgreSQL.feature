Feature: CWL-Airflow, DT_03 -  No Conda +Existing PostgreSQL
	As a user I want to work with CWL-Airflow on OS without conda and with existing container with PostgreSQL
	...
	Scenario: Copy env_example_postgres_noconda
		...
		When the user runs the command in bash shell "cp .env_example_nopostgres_noconda .env"
