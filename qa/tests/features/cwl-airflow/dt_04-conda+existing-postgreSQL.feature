Feature: CWL-Airflow, DT_04 -  Conda and existing PostgreSQL
	As a user I want to work with CWL-Airflow on OS with conda and with existing container with PostgreSQL
	...
	Scenario: Copy env_example_postgres_noconda
		...
		When the user runs the command in bash shell ".env_example_nopostgres_conda .env"
